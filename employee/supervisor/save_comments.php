...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $userId = isset($_SESSION['emploeee_id']) ? $_SESSION['employee_id'] : $_SESSION['a_id'];
    $sql = "SELECT firstname, lastname FROM employee_register WHERE emploee_id = ? UNION SELECT firstname, lastname FROM admin_register WHERE a_id = ?";
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssss", $employeeId, $comment, $username, $timestamp);
